...
| Dial → reasoning depth | **Reasoning Dial** action — cycles `model_reasoning_effort` in `~/.codex/config.toml` (Light → Medium → High → Extra High → Ultra) and shows the current level. Applies to **new** threads (the in-app shortcut approach for live changes proved unreliable). Works as a key (cycle) or a Stream Deck + encoder (rotate). |
